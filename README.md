...
Container holds an refference to services whitout making a direct call while register by taking advantage of using closures.
Services are registered but instansition have place when we get and than call the service from the "container". 
It is also posible to run service directly returning only the object and not a clousre function ($container->runService()).
